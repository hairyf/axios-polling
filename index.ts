 ...
 * @LastEditTime: 2022-01-25 15:23:15
 * @Description:
 * @LastEditors: Mr'Mao
...
export class AxiosPolling {
  constructor(private instance = axios as AxiosStatic | AxiosInstance) {}
  /** 循环调用模型 */
  ...
  emit = (observe: Observe, config: AxiosRequestConfig) => {
    ...
    const loopModel = this.loopCallModel(async () => {
      ...
      try {
        ...
        // 调用当前监视者失败堆栈
        observe['stacks']['error'].forEach((callback) => callback(error));
        // 当错误超出限制, 阻止 loop
 ...
 */
export const axiosPolling = (axios: AxiosStatic | AxiosInstance, config?: Partial<AxiosPollingConfig>) => {
  const defaultConfig = {
    count: 0,
    delay: 1000,
    delayGaps: 0,
